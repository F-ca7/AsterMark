...
public class POSInversionAlgorithm {
    private static MaxentTagger tagger = new MaxentTagger(MaxentTagger.DEFAULT_DISTRIBUTION_PATH);;
    ...
    public static String encodeTextByPOS(String text, int bit) {
        PTBTokenizer<CoreLabel> ptb = new PTBTokenizer<>(new StringReader(text), new CoreLabelTokenFactory(), null);
...
//                System.out.println("副词"+tw.word()+"  "+wordIndex);
        ...
        if (hasNoAdv){
            ...
            int toInserIndex = sequenceList.size()<1?1:sequenceList.get(0).getKey();
